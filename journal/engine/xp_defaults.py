...
def resolve_default_xp(
    ...
    per_type_weight = 1.0 / len(matched)
    combined: dict[str, float] = {}
    for entry_type in matched:
        for target in DEFAULT_XP_TARGETS[entry_type]:
            competency_id = str(target["competency_id"])
            weight = float(target["weight"]) * per_type_weight
            combined[competency_id] = combined.get(competency_id, 0.0) + weight

    targets = [
        {"competency_id": competency_id, "weight": round(weight, 4)}
        for competency_id, weight in combined.items()
    ]
    return DEFAULT_ENTRY_XP, targets
